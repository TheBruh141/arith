...
fn assert_eval_ok(input: &str, expected: f64) {
    ...
    match &results[0] {
        Ok(val) => assert_eq!(*val, expected, "Input: {}", input),
...
fn assert_eval_err(input: &str, expected_err_type: &str) {
    ...
    assert_eq!(results.len(), 1, "Expected one result for input: {}", input);
    match &results[0] {
        Ok(val) => panic!("Expected error for input '{}', but got: {}", input, val),
        Err(e) => {
...
fn test_empty_and_comment_lines() {
    ...
    let results = evaluate_lines("1 + 1\n; comment\n2 * 2");
...
fn test_invalid_syntax() {
    assert_eval_err("1 + * 2", "UnexpectedToken");
    assert_eval_err(" (1 + 2 ", "UnexpectedToken");
    assert_eval_err("abc", "UnexpectedCharacter");
    assert_eval_err("1.2.3", "UnexpectedCharacter");
}
...
fn test_long_expression() {
    ...
    assert_eval_ok(long_expr, -0.625);
}
...
fn test_multiple_lines_with_comments_and_empty() {
    ...
    assert_eq!(*results[0].as_ref().unwrap(), 2.0);
    assert_eq!(*results[1].as_ref().unwrap(), 4.0);
    assert_eq!(*results[2].as_ref().unwrap(), 6.0);
}
